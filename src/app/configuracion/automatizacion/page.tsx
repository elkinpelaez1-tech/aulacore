...
export default function AutomatizacionPage() {
  ...
  const [metrics, setMetrics] = useState({
    profilesAutoGenerated: 0,
    hoursSavedEstimated: 0,
    completionRate: 0,
  ...
  const handleUpdateDocumentStatus = (approvalId: string, docId: string, newStatus: 'Validado' | 'Pendiente' | 'Rechazado') => {
    setPendingApprovals(prev => prev.map(app => {
      ...
      const updatedDocs = app.documents.map((d: any) => {
      ...
      const allValid = updatedDocs.every((d: any) => d.status === 'Validado');
      const anyRejected = updatedDocs.some((d: any) => d.status === 'Rechazado');
