...
export async function POST(request: Request) {
  try {
    ...
    const { to, subject, message, html, category, recipientName } = await request.json();

    const emailBody = html || message;

    if (!to || !subject || !emailBody) {
      return NextResponse.json(
        { error: 'Destinatario, asunto y contenido (html o message) son obligatorios.' },
        { status: 400 }
      );
    }
