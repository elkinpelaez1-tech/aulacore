-- =========================================================================================
-- 🛡️ AULACORE ENTERPRISE - P0-05: ATOMIC SCHOOL PROVISIONING RPC (SINGLE ENTRY POINT)
-- =========================================================================================
...
--              Al ejecutarse bajo SECURITY DEFINER, la inserción ocurre de forma atómica
--              y segura sin requerir políticas RLS abiertas de escritura en el cliente web,
--              protegiendo las tablas base mediante Default Deny.
--              Incluye auditoría de trazas en motor (RAISE LOG) para diagnóstico forense
--              y validaciones de negocio en servidor (nombre obligatorio, slug único).
-- =========================================================================================

-- -----------------------------------------------------------------------------------------
-- 1. FUNCIÓN RPC TRANSACCIONAL ATÓMICA: create_school
-- -----------------------------------------------------------------------------------------
...
BEGIN
  ...
  -- 1. Validación estricta de sesión activa y rol Super Admin
  ...
  IF NOT public.is_super_admin() THEN
    RAISE LOG '[create_school] ERROR: Usuario % denegado por falta de rol super_admin.', auth.uid();
    RAISE EXCEPTION 'Acceso denegado. Solo un Super Administrador puede aprovisionar nuevas organizaciones.';
  END IF;
...
-- -----------------------------------------------------------------------------------------
-- 2. DOCUMENTACIÓN OFICIAL Y PERMISOS DE EJECUCIÓN (REVOKE / GRANT)
-- -----------------------------------------------------------------------------------------
COMMENT ON FUNCTION public.create_school(jsonb)
IS 'Único punto autorizado para aprovisionar nuevas instituciones en AulaCore. Valida estricta autenticación y rol super_admin antes de insertar de forma atómica bajo SECURITY DEFINER. Registra trazas en RAISE LOG y valida unicidad del slug.';
